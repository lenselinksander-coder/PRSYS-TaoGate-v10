      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <Button 
            ...
            disabled={!name.trim() || categories.length === 0 || saveMutation.isPending}
...
function ScopeCard({ scope, onEdit }: { scope: Scope; onEdit: (scope: Scope) => void }) {
  const queryClient = useQueryClient();
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {scopeList.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {scopeList.map(scope => (
            <ScopeCard key={scope.id} scope={scope} onEdit={setEditingScope} />
          ))}
        </div>
      )}
    </div>
